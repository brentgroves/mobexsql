...
BEGIN   
	...
	do
		...
		delete from Plex.accounting_balance WHERE pcn = cur_pcn and period between cur_period_start and cur_period_end;
		set cur_id = cur_id + 1;
    END WHILE; 
END; 
call Plex.accounting_balance_delete_period_range()

select distinct pcn,period from Plex.accounting_balance  order by pcn,period  --, 200812 to 202204

select count(*) from Plex.accounting_balance
select * from Plex.accounting_balance_update_period_range
