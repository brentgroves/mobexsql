...
--
--EXEC	@return_value = [Plex].[account_period_balance_recreate_period_range] 123681
--
...
( 	select a.*
	...
	on a.pcn = y.pcn 
	and a.account_no =y.account_no
	where y.pcn is null -- there is no account_year_category_type records for the @end_period year so we must add them.
	and a.pcn = @pcn
)
-- select * 
-- from Plex.accounting_account_year_category_type y
